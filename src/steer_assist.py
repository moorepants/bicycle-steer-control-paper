...
greys = mpl.colormaps['viridis']
gains = [-1.0, -5.0, -10.0, -50.0, -100.0, -500.0, -1000.0, -5000.0]
color_vals = np.linspace(0.2, 1.0, num=len(gains) + 1)
ax = model.plot_eigenvalue_parts(v=np.linspace(0.0, 10.0, num=2000),
                                 colors=[greys(color_vals[0])]*3)
for gain, color_val in zip(gains, color_vals[1:]):
    color_rgb = greys(color_val)
